...
addon = xbmcaddon.Addon(id='script.steamlink.launcher')
addonPath = addon.getAddonInfo('path')
addonIcon = addon.getAddonInfo('icon')
addonVersion = addon.getAddonInfo('version')
dialog = xbmcgui.Dialog()
language = addon.getLocalizedString
scriptid = 'script.steamlink.launcher'
...
def launchSteam():
	if osAndroid:
		...
		steamlauncher = os.path.join(scripts_path, 'steam-launcher.sh')
		cmd = '"%s" "%s" "%s" "%s" "%s" "%s" "%s" "%s" "%s" "%s" %s' % (steamlauncher, steamLinux, kodiLinux, quitKodiSetting, kodiPortable, preScript, postScript, steamParameters, forceKillKodi, desktopMode, "> " + steamlauncher.replace('.sh', '.log'))
	try:
		log('attempting to launch: %s' % cmd)
